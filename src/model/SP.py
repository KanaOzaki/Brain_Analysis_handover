...
def main():
    ...
    #テスト用脳活動行列の読み込み
    with open( DATA_DIR + '/Brain/' + target + '/' + sub + '_test_reduced_' + threshold +'.pickle', 'rb') as f:
    ...
    Dict = np.load( DATA_DIR + "/Dict/" + target + "/Dict_" + sub + "_pred" + threshold + "_base" + str(dimention) + "_sec" + str(shift) + "_sample" + str(sample) + ".pickle")
    # 脳活動辞書の部分だけ取り出す
    ...
    #係数保存
    f = open( DATA_DIR + "/Test/" + target + "/TestCoef_" + sub + "_pred" + threshold + "_base" + str(dimention) + "_sec" + str(shift) + "_sample" + str(sample) + ".pickle","wb")
    ...
    #推定意味表象行列保存
    f = open( DATA_DIR + "/Test/" + target + "/ESRM_" + sub + "_pred" + threshold + "_base" + str(dimention) + "_sec" + str(shift) + "_sample" + str(sample) + ".pickle","wb")
