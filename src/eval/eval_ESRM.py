...
def main():
	...
	# 正解意味表象行列の読み込み
	Correct_SRM = np.load( DATA_DIR + "/srm/" + target + "_srm300_test.pickle")

	# 推定意味表象行列の読み込み
	Estimate_SRM = np.load( DATA_DIR + "/Test/" + target +  "/ESRM_" + sub + "_pred" + threshold + "_base" + str(dimention) + "_sec" + str(shift) + "_sample" + str(sample) + ".pickle")
	...
	# サンプルごとのcos類似度保存(いちいち保存しなくても、標準出力でOK)
	#with open( DATA_DIR + "/cos_sim/" + target +  "/cos_sim" + sub + "_sec" + str(shift) + "_sample" + str(sample) + ".pickle", "wb") as f:
